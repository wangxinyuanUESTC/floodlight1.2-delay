...
public class OFConnection  implements IOFConnection, IOFConnectionBackend{
	...
	public void updateLatency(U64 latency) {
		if (latency == null) {
			...
			DecimalFormat decimalFormat = new DecimalFormat("#,##0.000000000000000");//格式化设置 不让其以科学计数法显示
//			logger.info(Time.time()+" "+System.currentTimeMillis()+"***Recording previously***ID为："+this.getDatapathId()+"单程延迟："+(OFChannelHandler.endTime-OFChannelHandler.strTime)/2.0);
			logger.info(decimalFormat.format(OFChannelHandler.strTime1)+decimalFormat.format(OFChannelHandler.endTime1));
			logger.info("***Recording previously***ID为："+this.getDatapathId()+"单程延迟："+decimalFormat.format((OFChannelHandler.endTime1-OFChannelHandler.strTime1)/2.0));
			logger.info("打印当前的Time："+decimalFormat.format(Time.time()));
...
//			logger.info("***Recording previously***ID为："+this.getDatapathId()+"权重单程时延："+latency.getValue());
//			logger.info("权重时延"+(double)(Math.round((OFChannelHandler.endTime-OFChannelHandler.strTime)*500.0)/1000.0));
//			logger.info("打印时间"+System.currentTimeMillis());
		}
	}
